...
const DocumentUploadModal = ({ isOpen, onClose, lead, onUploadSuccess, batchGovReceiptMode }) => {
  ...
  const [uploadedFiles, setUploadedFiles] = useState(null); // <-- new state
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // If operation user and any of draft/poa/ua is selected, upload to /upload-drafting
      if (user && user.role === 'operation' && (files.draft || files.poa || files.ua)) {
        const formData = new FormData();
        if (files.draft) formData.append('draft', files.draft);
        ...
        toast.success('Draft/POA/UA uploaded successfully!');
        if (onUploadSuccess) onUploadSuccess();
        onClose();
        setUploading(false);
        return;
      }
      // Otherwise, normal document upload
      const formData = new FormData();
      if (batchGovReceiptMode) {
        ...
        Object.keys(files).forEach(field => {
          if (['draft','poa','ua'].includes(field)) return; // skip these for non-operation upload
      ...
      const data = await response.json(); // get Cloudinary URLs
      setUploadedFiles(data.files); // show URLs in modal
      toast.success('Documents uploaded successfully!');
      if (onUploadSuccess) onUploadSuccess(data.files);
      // Do not close modal immediately, let user see URLs
      // onClose();
    } catch (error) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div className="relative w-full mx-2 rounded-2xl shadow-2xl bg-white overflow-y-auto max-h-[90vh] hide-scrollbar" style={{ maxWidth: '900px' }}>
        ...
        <div className="px-6 py-6">
          {uploadedFiles ? (
            <div className="mb-6">
              <h3 className="text-lg font-bold text-green-700 mb-2">Uploaded Files</h3>
              <ul className="space-y-2">
                {Object.entries(uploadedFiles).map(([field, value]) => {
                  if (Array.isArray(value)) {
                    return value.map((file, idx) => (
                      <li key={field + idx}>
                        <span className="font-semibold">{field}:</span> <a href={file.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">{file.name}</a>
                      </li>
                    ));
                  } else {
                    return (
                      <li key={field}>
                        <span className="font-semibold">{field}:</span> <a href={value.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">{value.name}</a>
                      </li>
                    );
                  }
                })}
              </ul>
              <button onClick={handleClose} className="mt-6 px-4 py-2 rounded bg-green-600 text-white font-semibold">Close</button>
            </div>
          ) : (
            <>
              {lead && (
              ...
              {/* Draft/POA/UA fields for operation user only */}
              {user && user.role === 'operation' && (
                <div className="bg-yellow-50 rounded-xl p-4 mb-6">
                  <h3 className="text-lg font-bold text-yellow-900 mb-2">Drafting Documents (Operation Only)</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full border border-gray-300 rounded-lg px-3 py-2 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-yellow-400 transition"
                      />
                      {files.draft && (
                        <p className="text-xs text-yellow-600 mt-1">✓ {files.draft.name}</p>
                      )}
                    </div>
                    <div>
                      ...
                      />
                      {files.poa && (
                        <p className="text-xs text-yellow-600 mt-1">✓ {files.poa.name}</p>
                      )}
                    </div>
                    <div>
                      ...
                      />
                      {files.ua && (
                        <p className="text-xs text-yellow-600 mt-1">✓ {files.ua.name}</p>
                      )}
                    </div>
                  </div>
                </div>
