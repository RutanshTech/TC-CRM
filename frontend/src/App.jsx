...
import CreateAdvocate from './pages/CreateAdvocate';
import AddLead from './pages/AddLead';
import YourSheet from './pages/YourSheet';
...
function App() {
  ...
  // Add this effect at the top level of the App component:
  useEffect(() => {
    const user = JSON.parse(localStorage.getItem('user'));
    if (user && user.role === 'employee' && user.access?.copy === false) {
  ...
  // Determine if user-select should be none (only for employees without copy access)
  const disableUserSelect = user && user.role === 'employee' && (!user.access || user.access.copy === false);

  return (
    <div className={disableUserSelect ? 'select-none' : ''} style={disableUserSelect ? { userSelect: 'none' } : {}}>
      ...
      <div className="flex-1 bg-gray-50 min-h-screen">
                ...
                <Routes>
                  <Route path="/" element={
                    user && user.role === 'operation' ? (
                      ...
                      <Navigate to="/advocate-leads" replace />
                    ) : (
                      <Dashboard sidebarCollapsed={sidebarCollapsed} />
                    )
                  } />
                  <Route path="/leads" element={
                    canAccessAdmin() || (user && user.role === 'operation') ? (
                      <Leads sidebarCollapsed={sidebarCollapsed} />
                    ) : (
                      <Navigate to={user && user.role === 'employee' ? "/your-leads" : "/"} replace />
                    )
                  } />
                  ...
                  <Route path="/your-sheet" element={
                    user && user.role === 'employee' ? (
                      <YourSheet sidebarCollapsed={sidebarCollapsed} />
                    ) : (
                      <Navigate to="/" replace />
                    )
                  } />
                  <Route path="/add-lead" element={
                    user && user.role === 'employee' && user.access?.leadAdd ? (
                      <AddLead sidebarCollapsed={sidebarCollapsed} />
                    ) : (
                      <Navigate to="/your-leads" replace />
                    )
                  } />
                  ...
                  />
                  <Route 
                    path="/operations-add-lead" 
                    element={
                      user && user.role === 'operation' ? (
                        <AddLead sidebarCollapsed={sidebarCollapsed} />
                      ) : (
                        <Navigate to="/operations-total-leads" replace />
                      )
                    } 
                  />
                  ...
                  {/* Create Advocate - accessible by admin and super-admin */}
                  <Route 
                    path="/create-advocate" 
                    element={
                      canAccessAdmin() ? (
                        <CreateAdvocate sidebarCollapsed={sidebarCollapsed} />
                      ) : (
                        <Navigate to="/" replace />
                      )
                    } 
        ...
        <ToastContainer position="top-right" autoClose={3001} hideProgressBar={false} newestOnTop closeOnClick pauseOnFocusLoss draggable pauseOnHover />
      </div>
    </div>
  );
}
