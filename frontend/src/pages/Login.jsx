...
export default function Login({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (role === 'employee') {
        res = await axios.post("http://localhost:3000/api/auth/employee-login", {
          email,
          password,
        });
      } else if (role === 'operation') {
        res = await axios.post("http://localhost:3000/api/auth/operation-login", {
          email,
          password,
        });
      } else {
      ...
      if (role === 'advocate' || (res.data.user && res.data.user.role === 'advocate')) {
        try {
          await axios.patch(
            `http://localhost:3000/api/advocates/${res.data.user._id}/status`,
            { isActive: true },
