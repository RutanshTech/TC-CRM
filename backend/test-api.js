...
async function testAPI() {
  try {
    ...
    const adminsResponse = await axios.get('http://localhost:3000/api/adminsget', {
    ...
    if (adminsResponse.data.length > 0) {
      ...
      try {
        const permissionsResponse = await axios.get(`http://localhost:3000/api/admin/${admin.adminId}/permissions`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        const updateResponse = await axios.patch(`http://localhost:3000/api/admin/${admin.adminId}/permissions`, {
