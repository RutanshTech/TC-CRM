...
// List all advocates
router.get('/advocates', authMiddleware, roleMiddleware(['admin', 'super-admin', 'employee', 'advocate']), async (req, res) => {
  try {
...
router.post('/advocates', authMiddleware, roleMiddleware(['admin', 'super-admin']), async (req, res) => {
  try {
    ...
    const user = await User.create({ name, email, password: hash, role: 'advocate', plainPassword: password });
