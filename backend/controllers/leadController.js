...
exports.updateLeadByEmployee = async (req, res) => {
  try {
    ...
    console.log('DEBUG updateLeadByEmployee:', {
      ...
      assignedToOperation: lead.assignedToOperation?.toString(),
      reqUserId: req.user.id,
      reqUser: req.user,
      assignedToType: typeof lead.assignedTo,
      assignedToOperationType: typeof lead.assignedToOperation,
      reqUserIdType: typeof req.user.id
    });
    
    // Assignment check removed for testing: allow all roles to update any lead
    // Update allowed fields for employees and operation users
    ...
    if (req.user.role === 'operation') {
      allowedFields.push('fillingText');
      allowedFields.push('fillingTextArray');
    }
    const updateData = {};
...
exports.uploadLeadFile = async (req, res) => {
  try {
    ...
    const fieldMap = {
      ...
      'batchGovReceiptFile': 'batchGovReceiptFile'
    };
    ...
    for (const file of req.files) {
      ...
      console.log(`Processing file: ${fieldName} -> ${modelField}`, {
      ...
      try {
        ...
        const fileData = { url: result.secure_url, name: file.originalname };
        
        if (modelField === 'additionalFiles') {
          lead.additionalFiles.push(fileData);
          if (!responseFiles[fieldName]) responseFiles[fieldName] = [];
          responseFiles[fieldName].push(fileData);
        } else {
          lead[modelField] = fileData;
          responseFiles[fieldName] = fileData;
        }
      } catch (err) {
        console.error(`Cloudinary upload error (${fieldName}):`, err);
        return res.status(500).json({ message: `Cloudinary upload error (${fieldName})`, error: err.message, details: err });
      }
    }
    ...
    if (error.message && error.message.includes('cloudinary')) {
      return res.status(500).json({ 
        ...
        error: 'Missing or invalid Cloudinary credentials',
        details: 'Make sure CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, and CLOUDINARY_API_SECRET are set in your .env file'
      });
    }
...
exports.sheetDistributionUploadAndAssign = async (req, res) => {
  try {
    ...
    // Naya logic: Har row bina kisi phone number check ke assign karo
    for (const row of leads) {
      employee.assignedSheets.push({ data: row });
    }
    await employee.save();
    res.json({ message: 'Sheet assigned successfully', totalAssigned: leads.length });
  } catch (error) {
...
exports.deleteAssignedSheetRow = async (req, res) => {
  try {
    ...
    // Extra debug log
    // console.log('Trying to delete row with id:', id, 'from employee:', employeeId, 'assignedSheets:', employee.assignedSheets.length);
    // Use String comparison for _id
    employee.assignedSheets = employee.assignedSheets.filter(row => String(row._id) !== String(id));
    const afterCount = employee.assignedSheets.length;
    await employee.save();
    if (beforeCount === afterCount) {
      return res.status(404).json({ message: 'Sheet row not found. (Debug: _id mismatch or already deleted)' });
    }
...
// Bulk delete assigned sheet rows for an employee
exports.bulkDeleteAssignedSheetRows = async (req, res) => {
  try {
    const { rowIds, employeeId } = req.body;
    if (!Array.isArray(rowIds) || !employeeId) {
      return res.status(400).json({ message: 'rowIds (array) and employeeId are required.' });
    }
    const Employee = require('../models/Employee');
    const employee = await Employee.findOne({ employeeId });
    if (!employee) return res.status(404).json({ message: 'Employee not found.' });
    const beforeCount = employee.assignedSheets.length;
    employee.assignedSheets = employee.assignedSheets.filter(row => !rowIds.includes(String(row._id)));
    const afterCount = employee.assignedSheets.length;
    await employee.save();
    const deletedCount = beforeCount - afterCount;
    res.json({ message: `Deleted ${deletedCount} rows.`, deletedCount });
  } catch (error) {
    res.status(500).json({ message: 'Failed to bulk delete rows', error: error.message });
  }
};
