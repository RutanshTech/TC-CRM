...
exports.getAllEmployees = async (req, res) => {
  try {
    ...
    // Check if Employee model has the find method
    console.log('Employee.find available:', typeof Employee.find === 'function' ? 'Yes' : 'No');
...
exports.updateEmployeeAccess = async (req, res) => {
  try {
    ...
    res.json({ message: 'Employee access updated successfully', employee });
  } catch (error) {
    res.status(500).json({ message: 'Access update failed', error: error.message });
  }
};
