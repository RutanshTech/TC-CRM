...
exports.createPaymentEntry = async (req, res) => {
  try {
    ...
    // Notify Operation user if lead is assigned to Operation
    if (leadId) {
      const lead = await Lead.findById(leadId);
      if (lead && lead.assignedToOperation) {
        // Send notification to Operation user
        await Notification.create({
          title: 'Payment Created for Your Lead',
          message: `A payment of ₹${payment.amount} has been created for a lead assigned to you.`,
          type: 'payment_created',
          priority: 'high',
          recipients: [lead.assignedToOperation],
          sender: req.user.id,
          relatedData: {
