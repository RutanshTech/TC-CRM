...
async function testEmployeeStatusUpdate() {
  try {
    ...
    const onlineResponse = await axios.patch('http://localhost:3000/api/employees/status', {
      status: 'online'
    }, {
    ...
    const offlineResponse = await axios.patch('http://localhost:3000/api/employees/status', {
      status: 'offline'
    }, {
      headers: { Authorization: `Bearer ${token}` }
    });
