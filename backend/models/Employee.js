...
const employeeSchema = new mongoose.Schema({
  ...
  access: {
    ...
    paymentClaim: { type: Boolean, default: false },
  ...
  assignedSheets: [
    {
      ...
      data: mongoose.Schema.Types.Mixed // stores the original sheet row
    }
  ],
}, { timestamps: true });
...
employeeSchema.methods.unblockEmployee = function() {
  ...
  const now = new Date();
  const hasActiveLeave = this.leaveApplications.some(leave =>
